...
import math
import fastmri
from sensitivity import SensitivityModel as SensUnet
...
class Edge_Net(nn.Module):
    def __init__(self, indim, hiddim, conv=default_conv, n_MSRB=3):
        ...
        # tail
        modules_tail = [nn.Conv2d(hiddim* (self.n_MSRB + 1), hiddim, 1, padding=0, stride=1), conv(hiddim, 2, kernel_size)]
...
class EEM(nn.Module):
    def __init__(self, C, C1, C2, num_heads, bias):
        ...
        self.ehead = nn.Conv2d(2, C2, kernel_size=3, stride=1, padding=1, groups=1, bias=bias)
...
class EAMRI(nn.Module):
    ...
    def __init__(self,
        ...
        # edgeatt module
        self.fuse1 = EAM(indim, 2, attdim, num_head, bias=False, shift=shift) 
        self.fuse2 = EAM(indim, 2, attdim, num_head, bias=False, shift=shift) 
        self.fuse3 = EAM(indim, 2, attdim, num_head, bias=False, shift=shift) 
        self.fuse4 = EAM(indim, 2, attdim, num_head, bias=False, shift=shift) 
    ...
    def forward(self, x1, y, m): # (image, kspace, mask) 
        ...
        """
        input:
            x1: (B, H, W, 2) zero-filled image
            g: (B, H, W, 2) grappa image
        ...
        # estimated sens map 
        sens_maps = self.sens_net(y, m)
       
        x1 = self.reduce(x1, sens_maps) 

        max_vals = x1.view(x1.shape[0], 2, -1).max(dim=2).values.view(x1.shape[0], 2, 1, 1)
        min_vals = x1.view(x1.shape[0], 2, -1).min(dim=2).values.view(x1.shape[0], 2, 1, 1)

        x1 = (x1 - min_vals) / (max_vals - min_vals)

        # image head 
        x1 = self.imHead(x1, y, m, sens_maps) #(B, 2, H, W)
        
        # first stage
        x2 = self.net1(x1, y, m, sens_maps) #(B, 2, H, W)
        e2 = self.edgeNet(x1) #(B, 1, H, W)
        x1 = self.fuse1(x2, e2, y, m, sens_maps)

        # second stage
        x2 = self.net2(x1, y, m, sens_maps) #(B, 2, H, W)
        e3 = self.edgeNet(x1) 
        x1 = self.fuse2(x2, e3, y, m, sens_maps)

        # third stage
        x2 = self.net3(x1, y, m, sens_maps) #(B, 2, H, W)
        e4 = self.edgeNet(x1) 
        x1 = self.fuse3(x2, e4, y, m, sens_maps)

        e2 = e2 * (max_vals - min_vals) + min_vals
        e3 = e3 * (max_vals - min_vals) + min_vals
        e4 = e4 * (max_vals - min_vals) + min_vals
        x1 = x1 * (max_vals - min_vals) + min_vals
