...
class DC_multicoil(nn.Module):
    ...
    def forward(self, xin, y, mask, sens_map): # image size, kspace, mask, sens_maps
        """
        xin: (B, C, 2, H, W) 사이즈가 1이라면???
        """

        print(f"In DC: {xin.shape}")
        
        assert xin.shape[1] == 2, "dc layer the last dimension of input x should be greater than 2" # 원래 shape[1]
        iScale = 1
        
        xin = T.expand_operator(xin.permute(0,2,3,1), sens_map, dim=1) #(B, coils, H, W, 2) # 이일단 수정해볼게 
        # xin = T.expand_operator(xin.permute(0, 2, 3, 4, 1), sens_map, dim=2) # (B, C, coils, H, W, 2) dim=1
        # print(f"xin: {xin.shape}")
        
        xin_f = T.fft2(xin,normalized=self.normalized, shift=self.shift) # (B, C, coils, H, W, 2)
        xGT_f = y # (B, coils, H, W, 2)

        # print(f"xin_f shape: {xin_f.shape}")
        # print(f"xGT_f shape: {xGT_f.shape}")
        # print(f"mask shape: {mask.shape}")


        xout_f = xin_f + (- xin_f + xGT_f) * iScale * mask
        xout = T.ifft2(xout_f, normalized=self.normalized, shift=self.shift) 
        xout = T.reduce_operator(xout, sens_map, dim=1) #(B, H, W, 2) 원래 dim=1
        
        # print(f"xout shape: {xout.shape}")

        return xout.permute(0,3,1,2).contiguous()
        # return xout.permute(0, 1, 4, 2, 3).contiguous()
